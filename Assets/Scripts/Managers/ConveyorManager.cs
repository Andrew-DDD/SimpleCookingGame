...
public class ConveyorManager : MonoBehaviour
{
    ...
    public class TrayEvent : UnityEvent<int> { }
    [HideInInspector] public CustomerEvent m_SpawnCustomerEvent = new CustomerEvent();
    [HideInInspector] public TrayEvent m_TrayFigureCountEvent = new TrayEvent();
    [HideInInspector] public UnityEvent m_TrayMoveToTrashEvent = new UnityEvent();
    //~Events
    //--------------------------------------------------------------
    ...
    //--------------------------------------------------------------
    //Varuables
    private bool conveyorIsWorking;
    ...
    //--------------------------------------------------------------
    //Get
    public bool ConveyorIsWorking { get { return conveyorIsWorking; } }
    public Tray GetCurrentTry() { return currentTry; }
    //~Get
    //--------------------------------------------------------------
    ...
    {
        m_SpawnCustomerEvent.RemoveAllListeners();
        m_TrayFigureCountEvent.RemoveAllListeners();
        m_TrayMoveToTrashEvent.RemoveAllListeners();
    }
    ...
    {
        if (!conveyorIsWorking) return;
        if (currentTry)
        {
            var valid_customers = 
                (from customer in customers where Enumerable.SequenceEqual(customer.GetFigures().OrderBy(t => t), currentTry.GetFigures().OrderBy(t => t)) select customer).ToArray();
            Customer valid_customer = null;
            Tray tray = currentTry;

            if (valid_customers.Length > 0) valid_customer = valid_customers[tray.transform.GetClosestObject(valid_customers)]; //<= Select nearest customer
            if (valid_customer)
            {
                ...
                tray.OnEditComplete(customer_pos)
                    .OnComplete(() => 
                    {
                        ...
                        {
                            if (valid_customer.CustomerIsWaiting) valid_customer.GetOrder(tray);
    ...
    public void EnterNewTray()
    {
        ...
        m_TrayFigureCountEvent.Invoke(0);
    }
    public void AddFigure(int figure)
    {
        ...
        if (currentTry)
        {
            ...
            m_TrayFigureCountEvent.Invoke(figureCount);
        }
    }
    private void TrayToTrash(Tray tray)
    {
        m_TrayMoveToTrashEvent.Invoke();
        tray.TrayToTrash();
    }
    #endregion
    ...
    {
        while (customers.Count < customerPoints.Length && conveyorIsWorking)
        {
            SpawnCustomer();
    ...
    private Customer SpawnCustomer()
    {
        ...
        m_SpawnCustomerEvent.Invoke(customer);

        customer.m_CustomerEvent.AddListener((arg) =>
        {
